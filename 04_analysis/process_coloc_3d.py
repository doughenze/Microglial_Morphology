...
def generate_gene_coloc_matrices(df, counts_matrix_full, micro_1, gene_col, coord_cols, genes, 
                                 permutation_method, transform_matrix, N_permutations=1, contact_radius=2):
    """
    Generate a cell type contact matrix. With the nearest integer rounded value from colocalization radius calculation
    """
    ...
    if permutation_method == 'no_permutation':
        ...
        for i in range(N_permutations):
            try:
                permutated_cell_counts = generate_transcript_positions_3d(counts_matrix_full, micro_1)#, n_iterations=1)
                ...
                permutated_cell_counts['z'] = (permutated_cell_counts['z'].astype(int) + 1) * 1.5
...
def process_experiment(experiment: str, morph_class: str | None = None) -> None:
    ...
    morph_classes = [morph_class] if morph_class else ['3', '4']

    # Iterate through each ordered_morph classification
    for morph_class in morph_classes: #ad_viz.obs['ordered_morph'].unique():
        ...
        for i in tqdm(range(len(ad_viz_morph)), desc=f"Processing {batch} - Morph {morph_class}"):
            ...
            gene_soma_coloc_counts_no_perm = generate_gene_coloc_matrices(
                counts_nuclei, counts_matrix_dapi, dapi_3d, gene_col, 
            ...
            merged_soma_coloc_counts_no_perm += gene_soma_coloc_counts_no_perm.astype(int)
            ...
            gene_branches_coloc_counts_no_perm = generate_gene_coloc_matrices(
                counts_non_nuclei, counts_matrix_iba1, non_dapi_3d, gene_col, 
                ['x', 'y', 'z'], genes, 'no_permutation', transform_matrix
            )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run 3-D colocalisation on one experiment."
    )
    parser.add_argument(
        "experiment_path",
        help="Path to the experiment you want to process",
    )
    parser.add_argument(
        "morph_class",
        nargs="?",
        choices=("3", "4"),
        help="Optional morph class to process (3 or 4). "
             "If omitted, the script processes both classes.",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.morph_class:
        print(f"Morph class: {args.morph_class}")

    process_experiment(args.experiment_path, args.morph_class)

...
def segment_plane_dapi(im, win=155, area_thresh=500):
...
def run_experiment(exp_path: Path,
    ...
    for idx in tqdm(range(len(ad_viz)), desc=f"[{batch}]  cells"):
        ...
        filled_raw  = segment_image(tile_raw,  201, foreground=True)    # 201 for max proj 155 for the individual planes
        filled_dapi = segment_image(tile_dapi, 155, foreground=True, dapi=True)
